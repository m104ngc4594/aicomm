...
impl AppState {
    ...
    /// check if agent name exists in a chat
    ...
    /// check if agent id exists in a chat
    ...
    pub async fn update_agent(
        ...
        if !self.agent_id_exists(chat_id, agent_id).await? {
            info!("Agent {} does not exist in chat {}", agent_id, chat_id);
...
mod tests {
    ...
    async fn create_agent_should_work() -> Result<()> {
        ...
        assert_eq!(agent.model, "deepseek-r1:32b");
        assert_eq!(agent.prompt, "You are a helpful assistant");
        assert_eq!(agent.args, serde_json::json!({}).into());
        Ok(())
    }
    ...
    async fn list_agents_should_work() -> Result<()> {
        ...
        assert_eq!(agents[0].args, serde_json::json!({}).into());
        Ok(())
    }
    ...
    async fn update_agent_should_work() -> Result<()> {
        ...
        let input = UpdateAgent::new(
            agent.id as u64,
            "Can you tell me the weather in Beijing?",
            HashMap::<String, String>::new(),
        );
        let agent = state
        ...
        assert_eq!(agent.prompt, "Can you tell me the weather in Beijing?");
        assert_eq!(agent.args, serde_json::json!({}).into());
        Ok(())
    }
}
