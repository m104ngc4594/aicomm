...
run-docker: kill-dockers
	$(DOCKER) run --entrypoint /app/chat-server --name chat -d -p 6688:6688 --mount type=bind,source=$(PWD)/fixtures/chat.yml,target=/app/chat.yml,readonly localhost/chat-server:latest
	...
	$(DOCKER) run --entrypoint /app/bot --name bot -d -p 6686:6686 --network host --mount type=bind,source=$(PWD)/fixtures/bot.yml,target=/app/bot.yml,readonly localhost/bot-server:latest
